...
class GraphGenerator {
    constructor() {
        ...
        const outputElement =
            document.getElementsByClassName("generator-output")[0]
        let clickCount = 0
        let mousePos = []
    ...
    addVertIfNecessary(newVert) {
        let vertIndex = this.vertices.findIndex(
            vert => vert[0] === newVert[0] && vert[1] === newVert[1]
        )
        if (vertIndex === -1) {
    ...
    undo() {
        ...
        lastEdge.forEach(vertIndex => {
            const referenceCount = this.edges.reduce(
                (total, edge) =>
                    edge[0] === vertIndex || edge[1] === vertIndex
                        ? total + 1
                        : total,
                0
            )
    ...
    snap(vert) {
        const target = this.vertices.find(
            curVert =>
                (curVert[0] - vert[0]) ** 2 + (curVert[1] - vert[1]) ** 2 <=
                SNAP_RADIUS ** 2
        )
    ...
    draw(ctx, mousePos) {
        ...
        this.edges.forEach((edge, i) => {
            ...
            const secondVert =
                i === this.edges.length - 1 && this.lineStarted
                    ? mousePos
